...
def init_db():
    ...
    for q in ["ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS currency TEXT DEFAULT 'USD'","ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS telegram_username TEXT","ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS paid_at TIMESTAMP","ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS currency_symbol TEXT DEFAULT '$'","ALTER TABLE agent35_trades ADD COLUMN IF NOT EXISTS hit_entry_at TIMESTAMP","ALTER TABLE agent35_trades ADD COLUMN IF NOT EXISTS closed_at TIMESTAMP","ALTER TABLE agent35_trades ADD COLUMN IF NOT EXISTS result_price FLOAT","ALTER TABLE agent35_trades ADD COLUMN IF NOT EXISTS auto_updated BOOLEAN DEFAULT FALSE","ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS sessions TEXT DEFAULT 'London,New York'""ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS referral_code TEXT",
...
def approve(pid):
    ...
    cur.execute("UPDATE agent35_users SET payment_status='approved'...
...
def dashboard():
    ...
    ref_widget = f"""
    <div class='card' style='border:1.5px solid #10b981'><div style='display:flex;justify-content:space-between'><div class='stat-label'>🎁 Refer 10 = Lifetime FREE</div><div style='background:#10b981;color:#000;font-weight:900;padding:3px 10px;border-radius:20px;font-size:13px'>{ref_count}/10</div></div><div style='background:#070d1a;border-radius:10px;height:14px;margin:12px 0'><div style='width:{progress}%;height:100%;background:#10b981;border-radius:10px'></div></div><input value='{ref_link}' readonly><div style='display:flex;gap:8px;margin-top:8px'><button class='btn' style='flex:1' onclick="navigator.clipboard.writeText('{ref_link}')">COPY LINK</button><a class='btn-outline' style='flex:1;background:#25D366;color:#000;font-weight:900;border:none' href="https://wa.me/?text=Join Agent35 {ref_link}" target='_blank'>WHATSAPP</a></div></div>
    """
    content = ref_widget + f"<div class='grid grid4'>...
