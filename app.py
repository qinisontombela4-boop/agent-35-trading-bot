...
def init_db():
    ...
    # === NEW REFERRAL COLUMNS ===
    "ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS referral_code TEXT",
    "ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS referred_by TEXT",
    "ALTER TABLE agent35_users ADD COLUMN IF NOT EXISTS referral_count INT DEFAULT 0"]:
        try: cur.execute(q)
        except: pass
    conn.commit()
    ...
    if not cur.fetchone():
        ...
        # give creator a referral code too
    ...
    conn.commit()
    # ensure all existing users have referral_code
...
def guide_page():
 ...
 <div class='warn'>⚠️ Recommended Minimum: R1000 Total<br>R500 for Bot + R500 to Fund Trading Account<br><span style='color:#10b981'>OR Refer 10 Friends = Lifetime FREE (R5000 Value)</span></div>
 ...
 <div class='step'><div style='display:flex;align-items:center'><div class='step-num'>3</div><h3>Create Agent 35 Profile</h3></div><p>Enter email + password on home page -> Ref auto-generated e.g. AG35-0829-AB12. Share it to earn Lifetime.</p><a class='btn-outline' href='/'>Create Profile Now</a></div>
...
def dashboard():
    ...
    ref_widget = f"""
    <div class='card' style='border:1.5px solid #10b981;background:linear-gradient(135deg,#0e1625,#0b1a13)'>
      <div style='display:flex;justify-content:space-between'><div class='stat-label'>🎁 Refer 10 = Lifetime FREE (R5000)</div><div style='background:#10b981;color:#000;font-weight:900;padding:3px 10px;border-radius:20px;font-size:13px'>{ref_count}/10</div></div>
      <div style='background:#070d1a;border-radius:10px;height:14px;margin:12px 0;border:1px solid #1e2d45'><div style='width:{progress}%;height:100%;background:#10b981;border-radius:10px'></div></div>
      <input id='refLink' value='{ref_link}' readonly style='font-size:12px'><div style='display:flex;gap:8px;margin-top:10px'><button class='btn' style='flex:1' onclick="navigator.clipboard.writeText('{ref_link}');this.innerText='COPIED!';setTimeout(()=>this.innerText='COPY LINK',2000)">COPY LINK</button><a class='btn-outline' style='flex:1;background:#25D366;color:#000;font-weight:900;border:none' href="https://wa.me/?text=Yo! I use Agent35 to scan EURUSD and Gold with 80% accuracy. Join with my link and we both level up: {ref_link}" target='_blank'>WHATSAPP</a></div>
      <div style='font-size:11px;margin-top:8px;color:#94a3b8'>Share. When friend pays + you approve, you get +1. At 10 auto Lifetime.</div>
      {f"<div style='background:#10b98122;border:1px solid #10b981;padding:8px;border-radius:8px;margin-top:10px;color:#10b981;font-weight:800;text-align:center'>🎉 LIFETIME UNLOCKED! {ref_count} referrals</div>" if ref_count>=10 else ""}
    </div>
    """
...
def tg_webhook():
    ...
    try:
        ...
        if 'callback_query' in data:
            ...
            if tr:
                if action=='took':
                    cur.execute("UPDATE agent35_trades SET status='took', hit_entry_at=NOW() WHERE id=%s", (tid,)); conn.commit(); send_telegram(chat_id, f"TOOK {tr['symbol']} active", trade_id=tid, stage="active")
                elif action=='skip':
                    cur.execute("UPDATE agent35_trades SET status='skipped' WHERE id=%s", (tid,)); conn.commit(); send_telegram(chat_id, f"Skipped {tr['symbol']}")
        ...
        if 'message' in data:
            ...
            row=cur.fetchone()
            if row:
                cur.execute("UPDATE agent35_users SET telegram_id=%s, telegram_username=%s WHERE email=%s", (str(chat_id), username, row['email'])); conn.commit(); send_telegram(chat_id, f"Linked! {row['email']}", trade_id=88888, stage="signal")
            else:
...
@app.route('/healthz')
def health(): return jsonify({"status":"ok","version":"V6.1-referral","utc":datetime.utcnow().isoformat()})
