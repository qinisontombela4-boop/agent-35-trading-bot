...
def init_db():
    ...
    conn.commit()
    # FORCE CREATE CREATOR - FIXES YOUR ISSUE
                ...
                if new and new!=tr['status']:
                    if new in ('win','loss'):
                        ...
                        if tr['telegram_id']:
                            ...
                            send_telegram(tr['telegram_id'], f"{'✅ WIN' if new=='win' else '❌ LOSS'} *{tr['symbol']} {new.upper()}* {tr['direction']}\nPNL: {cs}{round(pnl,2)}\n🤖 Journal Auto-Updated")
...
def master():
    ...
    if not session.get('is_creator'):
        conn=get_conn(); cur=conn.cursor(); cur.execute("SELECT is_creator FROM agent35_users WHERE email=%s", (session['email'],)); r=cur.fetchone(); cur.close(); conn.close()
        if not r or not r['is_creator']: return redirect('/dashboard')
    conn=get_conn(); cur=conn.cursor(); cur.execute("SELECT * FROM agent35_users ORDER BY created_at DESC"); users=cur.fetchall()
    cur.execute("SELECT * FROM agent35_payments ORDER BY created_at DESC"); pays=cur.fetchall()
    cur.execute("SELECT user_email, COUNT(*) as trades, COALESCE(SUM(pnl),0) as pnl FROM agent35_trades GROUP BY user_email"); pnl_map={row['user_email']:(row['trades'],row['pnl']) for row in cur.fetchall()}
    cur.close(); conn.close()
    users_html="".join([f"<tr><td>{u['email']}<br><span style='font-size:10px;color:#10b981'>Ref: {u['payment_ref'] or 'None'}</span><br><span style='font-size:10px'>TG: @{u['telegram_username'] or 'None'} ID:{u['telegram_id'] or 'No'}</span></td><td>{u['plan']}<br><span class='badge { 'win' if u['payment_status']=='approved' else 'loss'}'>{u['payment_status']}</span></td><td>{pnl_map.get(u['email'],(0,0))[1]:.2f} ({pnl_map.get(u['email'],(0,0))[0]} trades)</td><td>{u['created_at'].strftime('%Y-%m-%d')}<br>Paid: {u['paid_at'].strftime('%Y-%m-%d') if u['paid_at'] else 'Not'}</td><td><a class='btn' style='padding:6px 10px;font-size:11px' href='/approve-user?email={u['email']}'>Approve</a></td></tr>" for u in users])
    pays_html="".join([f"<div class='card' style='margin:8px 0;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap'><div><b>{p['user_email']}</b><br>Ref: <b style='color:#10b981'>{p['ref_code']}</b> - R{p['amount']} - {p['plan']} - {p['status']}</div><div style='display:flex;gap:6px'><a class='btn' style='width:auto;padding:6px 12px' href='/approve/{p['id']}'>Approve Payment</a><a class='btn-outline' style='width:auto;padding:6px 12px' href='/approve-user?email={p['user_email']}'>Approve User</a></div></div>" for p in pays]) or "<p style='color:#475569'>No payments yet</p>"
    return layout(f"<h2 style='color:#10b981'>👑 MASTER DASHBOARD - FIXED</h2><div class='grid grid4'><div class='card'>Users: {len(users)}</div><div class='card'>Payments: {len(pays)}</div><div class='card'><a class='btn' href='/cron/update-trades'>Update All Trades</a></div><div class='card'><a class='btn-outline' href='/setup-webhook'>Setup Telegram Webhook</a></div></div><div class='card' style='margin-top:14px'><h3>💳 Payments (Approve to activate)</h3>{pays_html}</div><div class='card' style='margin-top:14px;overflow:auto'><h3>👥 All Users - Payment Ref Links Telegram</h3><p style='font-size:12px;color:#94a3b8'>User can link Telegram by sending their <b>payment ref</b> (AG35-xxxx) OR email to @{TELEGRAM_BOT_USERNAME} - Bot will auto-link.</p><table><tr><th>User / Ref / TG</th><th>Plan/Status</th><th>PNL</th><th>Joined/Paid</th><th>Action</th></tr>{users_html}</table></div>", session['email'], "master")
...
def submit_payment():
    ...
    conn.commit(); cur.close(); conn.close()
    return layout(f"<div class='card' style='text-align:center'><h2 style='color:#10b981'>✅ Payment Submitted</h2><p>Ref: <b>{ref}</b></p><p>Plan: {plan} - R{amount}</p><div style='background:#070d1a;padding:12px;border-radius:10px;margin:12px 0'><p style='font-size:13px'>Now link Telegram:</p><p style='font-size:12px'>1. Open Telegram @{TELEGRAM_BOT_USERNAME}<br>2. Send: <b>{ref}</b><br>3. Bot will auto-link you</p><a href='https://t.me/{TELEGRAM_BOT_USERNAME}?start={ref}' target='_blank' class='btn' style='margin-top:10px'>Open @{TELEGRAM_BOT_USERNAME} & Send Ref</a></div><a class='btn-outline' href='/dashboard'>Back to Dashboard</a></div>", session['email'])
...
def dashboard():
    ...
    return layout(f"<div class='grid grid4'><div class='card'><div class='stat-label'>PNL Auto</div><div class='stat-value'>{curr_sym}{round(pnl,2)} <span class='badge bull'>{round(wr,1)}% WR</span></div><div style='font-size:11px;color:#10b981'>Payment Ref: {user['payment_ref'] or 'None - Go to Plans'}</div></div><div class='card'><div class='stat-label'>Account {user['currency']} - {user['payment_status']}</div><div class='stat-value' style='font-size:18px'>{curr_sym}{user['account_size']}</div><a href='/settings' class='btn-outline'>Settings</a></div><div class='card' style='position:relative'><div class='stat-label'>Watchlist</div><div>{chips}</div><form id='symForm' method='POST' action='/quick-symbols'><input type='hidden' name='symbols' id='symInput' value=\"{user['symbols']}\"></form><input id='symSearch' class='searchbox' placeholder='Search...' oninput='filterSyms()'><div id='symDropdown' class='dropdown'></div></div><div class='card'><div class='stat-label'>Telegram @{TELEGRAM_BOT_USERNAME}</div><a class='btn' href='/scan'>SCAN</a><a href='https://t.me/{TELEGRAM_BOT_USERNAME}?start={user['payment_ref'] or session['email']}' target='_blank' class='btn-outline' style='border-color:#10b981;color:#10b981'>📲 Link via Ref: {user['payment_ref'] or session['email']}</a><a href='/test-telegram' class='btn-test'>🧪 Test Telegram</a></div></div><div class='card' style='margin-top:14px'><h3 style='margin:0;color:#10b981'>Recent Trades</h3><div style='overflow:auto'><table><tr><th>Time</th><th>Symbol</th><th>Status</th><th>PNL</th></tr>{''.join([f'<tr><td>{t[\"created_at\"].strftime(\"%m-%d %H:%M\")}</td><td>{t[\"symbol\"]}</td><td>{t[\"status\"]}</td><td>{curr_sym}{round(t[\"pnl\"],2)}</td></tr>' for t in trades])}</table></div></div>", session['email'], "dashboard")

@app.route('/test-telegram')
def test_telegram():
    if 'email' not in session: return redirect('/')
    conn=get_conn(); cur=conn.cursor(); cur.execute("SELECT telegram_id, account_size FROM agent35_users WHERE email=%s", (session['email'],)); u=cur.fetchone(); cur.close(); conn.close()
    if not u or not u['telegram_id']: return layout(f"<div class='card' style='text-align:center'><h3 style='color:#ef4444'>❌ Not Linked</h3><p>Send your payment ref {session['email']} to @{TELEGRAM_BOT_USERNAME}</p><a class='btn' href='https://t.me/{TELEGRAM_BOT_USERNAME}' target='_blank'>Open Bot</a><a class='btn-outline' href='/dashboard'>Back</a></div>", session['email'])
    ok = send_telegram(u['telegram_id'], f"🧪 *TEST OK* Agent35 @{TELEGRAM_BOT_USERNAME}\nYour Telegram is linked via payment ref!\nReal signals will come here.")
    return layout(f"<div class='card' style='text-align:center'><h3 style='color:#10b981'>✅ Sent to Telegram</h3><a class='btn' href='/dashboard'>Back</a></div>" if ok else f"<div class='card'><h3>Failed - Check TELEGRAM_BOT_TOKEN</h3><a class='btn-outline' href='/dashboard'>Back</a></div>", session['email'])
...
def journal():
    ...
    cur.execute("SELECT COALESCE(SUM(pnl),0) as total, COUNT(*) as total_trades FROM agent35_trades WHERE user_email=%s", (session['email'],)); s=cur.fetchone(); cur.close(); conn.close()
    curr_sym=CUR.get(user['currency'],'$')
    return layout(f"<div class='grid grid4'><div class='card'><div class='stat-label'>Total PNL</div><div class='stat-value'>{curr_sym}{round(s['total'],2)}</div></div><div class='card'><div class='stat-label'>Trades</div><div class='stat-value'>{s['total_trades']}</div></div><div class='card'><a href='/export/csv' class='btn'>📥 CSV Export</a></div><div class='card'><a href='/test-telegram' class='btn-test'>🧪 Test Telegram</a></div></div><div class='card' style='margin-top:14px;overflow:auto'><table><tr><th>Date</th><th>Symbol</th><th>Dir</th><th>Status</th><th>PNL</th></tr>{''.join([f'<tr><td>{t[\"created_at\"].strftime(\"%Y-%m-%d\")}</td><td>{t[\"symbol\"]}</td><td>{t[\"direction\"]}</td><td>{t[\"status\"]}</td><td>{curr_sym}{round(t[\"pnl\"],2)}</td></tr>' for t in trades])}</table></div>", session['email'], "journal")

@app.route('/export/csv')
def export_csv():
    if 'email' not in session: return redirect('/')
    conn=get_conn(); cur=conn.cursor(); cur.execute("SELECT * FROM agent35_users WHERE email=%s", (session['email'],)); user=cur.fetchone(); cur.execute("SELECT * FROM agent35_trades WHERE user_email=%s ORDER BY created_at DESC", (session['email'],)); trades=cur.fetchall(); cur.close(); conn.close()
    output=io.StringIO(); writer=csv.writer(output); writer.writerow(['Date','Symbol','Direction','Entry','SL','TP','Result','Status','PNL','Ref']); [writer.writerow([t['created_at'],t['symbol'],t['direction'],t['entry'],t['sl'],t['tp'],t['result_price'],t['status'],t['pnl'],user['payment_ref']]) for t in trades]
...
def scan():
    ...
    for sym in symbols:
        ...
        if res.get('signal'): cur.execute("INSERT INTO agent35_trades (user_email,symbol,direction,entry,sl,tp,timeframe_bias,confluence) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",(session['email'],res['symbol'],res['direction'],res['entry'],res['sl'],res['tp'],res['bias'],res['confluence']))
    cur.execute("UPDATE agent35_users SET last_scan_at=NOW() WHERE email=%s",(session['email'],)); conn.commit(); cur.close(); conn.close()
    html="".join([f"<div class='card' style='border-left:4px solid #10b981;margin:10px 0'><b>{r['symbol']} {r.get('direction','')}</b> {r.get('entry','')} SL {r.get('sl','')} TP {r.get('tp','')}</div>" if r.get('signal') else f"<div class='card' style='opacity:0.6;margin:10px 0'><b>{r['symbol']} - No Setup</b></div>" for r in results])
...
def cron_update():
    try:
        conn=get_conn(); cur=conn.cursor(); cur.execute("SELECT t.*, u.account_size, u.risk_reward, u.telegram_id, u.currency_symbol FROM agent35_trades t JOIN agent35_users u ON u.email=t.user_email WHERE t.status IN ('sent','took') LIMIT 30")
        for tr in cur.fetchall():
            live=get_live_price(tr['symbol'])
            if not live: continue
            close,high,low=live; rr=3
...
def settings():
    ...
    if request.method=='POST':
        conn=get_conn(); cur=conn.cursor(); cur.execute("UPDATE agent35_users SET symbols=%s, risk_reward=%s, account_size=%s, lot_size=%s, currency=%s, telegram_username=%s WHERE email=%s",(request.form['symbols'][:100], request.form['rr'], float(request.form['acc']), float(request.form['lot']), request.form['currency'], request.form['tg'], session['email'])); conn.commit(); cur.close(); conn.close(); return redirect('/dashboard')
    ...
    return layout(f"<div style='max-width:700px;margin:auto'><h2 style='color:#10b981'>Settings</h2><form method='POST'><div class='settings-section'><label>Currency</label><select name='currency'><option {'selected' if u['currency']=='USD' else ''}>USD</option><option {'selected' if u['currency']=='ZAR' else ''}>ZAR</option></select><label>Account Size</label><input name='acc' value='{u['account_size']}'><label>Lot</label><input name='lot' value='{u['lot_size']}'><label>RR</label><select name='rr'><option {'selected' if u['risk_reward']=='1:2' else ''}>1:2</option><option {'selected' if u['risk_reward']=='1:3' else ''}>1:3</option><option {'selected' if u['risk_reward']=='1:4' else ''}>1:4</option></select><label>Symbols</label><input name='symbols' value='{u['symbols']}'><label>Telegram @</label><input name='tg' value='{u['telegram_username'] or ''}'><div style='margin-top:10px'><div>Payment Ref: <b style='color:#10b981'>{u['payment_ref'] or 'None - Go to Plans page'}</b></div><div style='font-size:11px;color:#94a3b8'>Send this ref to @{TELEGRAM_BOT_USERNAME} to link</div><a href='https://t.me/{TELEGRAM_BOT_USERNAME}?start={u['payment_ref'] or session['email']}' target='_blank' class='btn-outline' style='border-color:#10b981;color:#10b981'>Link via Ref {u['payment_ref'] or ''}</a></div></div><button class='btn'>Save</button></form></div>", session['email'], "settings")

# --- TELEGRAM VIA PAYMENT REF - NEW FEATURE ---
...
def tg_webhook():
    ...
    try:
        if 'message' in data:
            ...
            ref = None
            if '/start' in text: ref = text.split('/start')[-1].strip()
            else: ref = text.strip() # user can just send ref directly
            if not ref: return jsonify({"ok":True})
            conn=get_conn(); cur=conn.cursor()
            # Try by payment_ref first (NEW), then by email
            ...
            if row:
                ...
                conn.commit()
                send_telegram(chat_id, f"✅ *Linked!* {row['email']}\nPayment Ref: {ref}\nBot: @{TELEGRAM_BOT_USERNAME}\nYou will now receive signals and auto close alerts.\n\nUse /test in dashboard to test.")
            else:
                # Not found - ask to pay
                send_telegram(chat_id, f"❌ Ref *{ref}* not found.\nMake sure you paid and used ref from /payment page.\nYour ref looks like AG35-XXXX-XXXX\n\nThen send that ref here.")
            cur.close(); conn.close()
...
def setup_webhook():
    ...
    if not TELEGRAM_TOKEN: return "No TELEGRAM_BOT_TOKEN set in Render"
    base = request.host_url.rstrip('/')
    wh_url = f"{base}/telegram/webhook"
    r = requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/setWebhook?url={wh_url}")
    return layout(f"<div class='card'><h3>Webhook Setup</h3><p>URL: {wh_url}</p><p>Response: {r.text}</p><a class='btn' href='/master'>Back</a></div>", session['email'])

@app.route('/healthz')
def health(): return jsonify({"status":"ok","master":"fixed","payment":"fixed","telegram_ref":"enabled","bot":"@Sniper035_bot"})
