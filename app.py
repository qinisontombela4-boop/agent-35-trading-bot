...
load_dotenv()
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET", "agent35-v19-1-fixed")
...
ALL_SYMBOLS = ["EURUSD","GBPUSD","USDJPY","USDCHF","AUDUSD","USDCAD","NZDUSD","EURJPY","GBPJPY","EURGBP","AUDJPY","CADJPY","XAUUSD","XAGUSD","US30","NAS100","SPX500","GER40","BTCUSD","ETHUSD","GBPCHF","EURCHF"]
...
def dashboard():
    ...
    content=f"<div class='main'><div class='card'><div class='card-title'>Total Profit</div><div class='card-value' style='color:#ef4444'>R{info.get('total_profit',-1.42)}</div><div style='background:#0f172a;border-radius:12px;padding:12px;margin-top:12px;font-size:11px;color:#94a3b8'>Lot: {user_settings.get('lot_size')} | Lev: {user_settings.get('leverage')}<br>Telegram: <span style='color:{tg_color};font-weight:700'>{tg_status}</span><br>Referrals: {ref_count}/10</div></div><div class='card'><div class='card-title'>Account</div><div class='card-value' style='color:white'>R{user_settings.get('account_size',142)}</div><a href='/settings' style='color:#10b981;text-decoration:none;font-weight:700;font-size:12px'>Edit Settings</a></div><div class='card'><div class='card-title'>Watchlist ({len(user_settings.get('symbols',[]))})</div><div style='display:flex;flex-wrap:wrap;gap:6px;margin:12px 0'>{''.join([f\"<span style='background:#1e293b;border:1px solid #334155;padding:6px 10px;border-radius:20px;font-size:11px'>{s}</span>\" for s in user_settings.get('symbols',[])[:8]])}</div><div style='font-size:11px;color:#94a3b8'>Last: {str(system.get('last_scan','Never'))[:16]} | Track {len([t for t in tracked.values() if t['status']=='TRACKING'])}</div></div><div class='card' style='background:linear-gradient(145deg,#132a22 0%,#1e293b 100%)'><a href='/dashboard-scan'><button class='btn-primary'>SCAN NOW</button></a><a href='/test-telegram' class='btn-secondary'>Test Telegram V19</a><a href='/link-telegram' class='btn-secondary'>Link Telegram Chat ID</a>{f\"<a href='/creator?secret=' class='btn-secondary' style='background:#f59e0b;color:white;font-weight:700'>Creator</a>\" if is_admin else \"\"}</div></div><div style='max-width:1600px;margin:0 auto;padding:0 20px 20px'><div class='table-card'><h3>Recent Trades</h3><table><tr><th>Time</th><th>Symbol</th><th>Status</th><th>Result</th></tr>{rows}</table></div></div>"
...
def referral_page():
    ...
    refs_html="".join([f"<tr><td>{r['email'][:28]}</td><td><span style='color:{'#10b981' if 'PAID' in r['status'] else '#f59e0b'}'>{r['status']}</span></td><td style='font-size:11px'>{r['plan'][:20]}</td></tr>" for r in my_refs]) or "<tr><td colspan=3 style='text-align:center;padding:20px;color:#64748b'>No referrals yet</td></tr>"
    link=f"https://agent-35-trading-bot.onrender.com/register?ref={my_code}"
    content=f"<div style='max-width:1000px;margin:0 auto;padding:20px'><h1>Referral - 10 Paid = Lifetime FREE</h1><div style='display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:20px'><div class='card' style='background:linear-gradient(145deg,#132a22 0%,#1e293b 100%)'><div class='card-title'>Your Code</div><div style='background:white;color:black;padding:16px;border-radius:12px;font-weight:900;font-size:24px;text-align:center;margin:12px 0'>{my_code}</div><div style='background:#0f172a;padding:10px;border-radius:10px;font-size:11px;word-break:break-all;color:#94a3b8'>{link}</div><a href='https://wa.me/?text={link}' target='_blank' style='background:#25D366;color:white;padding:10px;border-radius:10px;text-align:center;display:block;text-decoration:none;font-weight:700;margin-top:10px'>WhatsApp Share</a></div><div class='card'><div class='card-title'>Progress {paid_count}/10</div><div style='background:#0f172a;border-radius:20px;height:14px;overflow:hidden;margin:12px 0'><div style='background:linear-gradient(90deg,#10b981,#059669);width:{progress}%;height:100%'></div></div><div style='font-size:12px;color:#94a3b8'>Valid ONLY if referee PAID R500/R5000<br>Auto FREE at 10</div>{'<div style=\"background:#10b981;color:white;padding:12px;border-radius:12px;text-align:center;font-weight:900;margin-top:12px\">FREE LIFETIME UNLOCKED</div>' if paid_count>=10 else ''}</div></div><div class='table-card' style='margin-top:20px'><h3>Your Referrals ({len(my_refs)})</h3><table><tr><th>Email</th><th>Status</th><th>Plan</th></tr>{refs_html}</table></div></div>"
...
def link_telegram():
    ...
    content=f"<div style='max-width:700px;margin:0 auto;padding:20px'><div class='card'><h2>Link Telegram</h2><div style='background:#0f172a;padding:12px;border-radius:12px;margin:12px 0'>Current: <span style='color:#10b981;font-weight:700'>{current}</span><br>Email: {email}</div><a href='https://t.me/Sniper035_bot' target='_blank' style='background:#0088cc;color:white;padding:14px;border-radius:12px;text-align:center;display:block;text-decoration:none;font-weight:800'>Open @Sniper035_bot</a><h3 style='color:#f59e0b;margin-top:20px'>Manual Add Chat ID</h3><form method='post'><input name='chat_id' placeholder='Paste Chat ID e.g. 123456789' style='width:100%;padding:14px;border-radius:12px;border:1px solid #334155;background:#0f172a;color:white' required><button type='submit' style='background:#f59e0b;color:white;padding:14px;width:100%;border:none;border-radius:12px;font-weight:800;margin-top:10px'>Save Chat ID Manually</button></form></div></div>"
...
def dashboard_scan():
    ...
    for s in symbols_to_scan[:30]:
        try:
            ...
            rows+=f"<tr><td style='font-weight:800'>{s}</td><td><span style='background:{color}22;color:{color};padding:4px 10px;border-radius:20px;font-weight:800'>{score}/8</span></td><td>{bias}</td><td>{signal}</td><td><a href='/send-signal?symbol={s}' style='background:#10b981;color:white;padding:6px 12px;border-radius:8px;text-decoration:none;font-weight:700;font-size:12px'>Send TG</a></td></tr>"
        except Exception as e:
            rows+=f"<tr><td>{s}</td><td colspan=4 style='font-size:11px;color:#ef4444'>{str(e)[:80]}</td></tr>"
    content=f"<div style='max-width:1400px;margin:0 auto;padding:20px'><div class='table-card'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:12px'><h2>Signals - {len(symbols_to_scan)} Pairs</h2><form method='get' style='display:flex;gap:8px'><input name='q' value='{q}' placeholder='Search EURUSD, GOLD, BTC...' class='search-box' style='width:220px'><button style='background:#10b981;color:white;padding:10px 16px;border-radius:10px;border:none;font-weight:700'>Search</button></form></div><div style='overflow-x:auto'><table><tr><th>Symbol</th><th>Score</th><th>Bias</th><th>Signal</th><th>TG V19</th></tr>{rows}</table></div></div></div>"
...
def settings_page():
    ...
    symbols_html="".join([f"<label style='display:flex;align-items:center;gap:8px;background:#0f172a;border:1px solid #1e293b;padding:10px 12px;border-radius:10px;font-size:13px'><input type='checkbox' name='symbols' value='{sym}' {'checked' if sym in s.get('symbols',[]) else ''}> {sym}</label>" for sym in ALL_SYMBOLS])
    ...
    content=f"<div style='max-width:900px;margin:0 auto;padding:20px'><h1>Trading Settings</h1><form method='post'><div style='display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:20px'><div class='card'><div class='card-title'>Account Size R</div><input name='account_size' type='number' step='0.01' value='{s.get('account_size',142)}' style='width:100%;padding:14px;margin-top:10px;border-radius:12px;border:1px solid #334155;background:#0f172a;color:white'></div><div class='card'><div class='card-title'>Lot Size</div><input name='lot_size' type='number' step='0.01' value='{s.get('lot_size',0.01)}' style='width:100%;padding:14px;margin-top:10px;border-radius:12px;border:1px solid #334155;background:#0f172a;color:white'></div></div><div class='card' style='margin-top:16px'><div class='card-title'>Sessions</div><div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:10px'>{sessions_html}</div></div><div class='card' style='margin-top:16px'><div class='card-title'>Symbols ({len(ALL_SYMBOLS)}) Searchable</div><div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px;margin-top:10px;max-height:400px;overflow-y:auto'>{symbols_html}</div></div><button type='submit' class='btn-primary' style='margin-top:20px'>SAVE SETTINGS</button></form><a href='/link-telegram' style='background:#0088cc;color:white;padding:14px;border-radius:12px;text-align:center;display:block;text-decoration:none;margin-top:16px'>Manage Telegram Chat ID</a></div>"
    return pro_layout(content,"Settings", is_admin=is_admin)
...
def send_signal():
    ...
    try:
        ...
        if "USD" not in sym:
            entry_val = round(random.uniform(100, 2000), 2)
        entry = str(entry_val)
        sl = str(round(float(entry_val)+0.001, 5))
        tp = str(round(float(entry_val)-0.0025, 5))
        conf=f"· HTF: Daily PREMIUM 71% | 4H PREMIUM 92%\n· ✅ 4H PREMIUM 92% aligned\n· 🔥 5M: Price in PREMIUM 71%\n· ✅ Daily PREMIUM 71%"
        res=send_telegram_pro(sym, r.get('score',5), r.get('bias','BUY'), entry, sl, tp, "1:2.5", "0.75", conf)
        msg=f"Sent {sym}"
    except Exception as e:
        res={"error":str(e)}
        msg="Error"
...
def test_telegram():
    ...
    return f"<html><body style='background:#080c14;color:white;padding:40px'><div style='max-width:600px;margin:auto;background:#1e293b;padding:24px;border-radius:20px'><h2>V19 Test Sent</h2><p style='font-size:11px;background:#0f172a;padding:10px;border-radius:8px'>{str(res)[:2000]}</p><a href='/creator?secret=' style='background:#10b981;color:white;padding:10px 20px;border-radius:12px;text-decoration:none'>Back</a></div></body></html>"
...
def telegram_webhook():
    ...
    if "message" in data:
        ...
        if bot and "/start" in data["message"].get("text",""):
            try:
                requests.post(f"https://api.telegram.org/bot{bot}/sendMessage", json={"chat_id":chat_id,"text":f"Welcome Agent 35 V19\nYour Chat ID: {chat_id}\nGo dashboard paste in Link Telegram if not auto linked."}, timeout=5)
            except:
                pass
    if "callback_query" in data:
        ...
        if cb_data.startswith("TOOK_"):
            ...
            entry=cb_data.split("_")[2] if len(cb_data.split("_"))>2 else ""
...
def journal_page():
    ...
    journal=load_json(JOURNAL_FILE, list)
    q=request.args.get("q","").upper()
    filtered=[j for j in journal if q in j.get("symbol","").upper()] if q else journal
    ...
    content=f"<div style='max-width:1400px;margin:0 auto;padding:20px'><h1>Journal ({len(filtered)})</h1><form method='get'><input name='q' value='{q}' placeholder='Search symbol' class='search-box' style='width:200px'><button style='background:#10b981;color:white;padding:10px;border-radius:10px;border:none'>Search</button></form><div class='table-card'><table><tr><th>Time</th><th>Symbol</th><th>Status</th><th>Result</th><th>Date</th></tr>{rows}</table></div></div>"
...
def creator_dashboard():
    ...
    pending_rows="".join([f"<tr><td>{ref}</td><td>{pay.get('user','')}</td><td>R{pay.get('price','')}</td><td>{pay.get('referred_by','')}</td><td><a href='/creator/action?act=approve_payment&ref={ref}&secret={secret}' style='background:#10b981;color:white;padding:6px 12px;border-radius:8px;text-decoration:none'>APPROVE</a> <a href='/creator/action?act=decline_payment&ref={ref}&secret={secret}' style='background:#ef4444;color:white;padding:6px 10px;border-radius:8px;text-decoration:none'>DECLINE</a></td></tr>" for ref,pay in pending.items()]) or "<tr><td colspan=5 style='text-align:center;color:#64748b'>No pending</td></tr>"
    content=f"<div style='max-width:1600px;margin:0 auto;padding:20px'><h1>Creator V19 Fixed</h1><div class='table-card' style='border:2px solid #f59e0b44'><h3>PENDING ({len(pending)})</h3><table><tr><th>Ref</th><th>Email</th><th>Price</th><th>Referred By</th><th>Action</th></tr>{pending_rows}</table></div><div style='margin-top:16px;display:flex;gap:8px'><a href='/creator/webhook?action=set&secret={secret}' style='background:#10b981;color:white;padding:12px;border-radius:10px;text-decoration:none;font-weight:700'>SET WEBHOOK</a><a href='/test-telegram' style='background:#1e293b;border:1px solid #334155;color:white;padding:12px;border-radius:10px;text-decoration:none'>Test V19</a></div></div>"
...
@app.route("/pay")
def pay_page():
    return "<div style='max-width:500px;margin:0 auto;padding:20px'><div class='card'><h2>Buy Plan</h2><form action='/pay/create' method='get'><input name='user' placeholder='Email' required style='width:100%;padding:14px;margin:6px 0;border-radius:12px;background:#0f172a;color:white;border:1px solid #334155'><input name='phone' placeholder='WhatsApp' required style='width:100%;padding:14px;margin:6px 0;border-radius:12px;background:#0f172a;color:white;border:1px solid #334155'><select name='plan' style='width:100%;padding:14px;border-radius:12px;background:#0f172a;color:white'><option value='yearly'>Yearly R500</option><option value='lifetime'>Lifetime R5000</option></select><button style='background:#10b981;color:white;padding:14px;width:100%;border:none;border-radius:12px;margin-top:10px'>GET REF</button></form></div></div>"
...
def guide_page():
    ...
    content="<div style='max-width:900px;margin:0 auto;padding:20px'><h1>Guide V19 Fixed</h1><div class='card'><p>V19 Fixed deploy + Telegram like screenshot TOOK/SKIP WIN/LOSS/BE + Referral 10 auto FREE + Manual TG Chat ID</p></div></div>"
    ...
    for sym in ["EURUSD","GBPUSD","XAUUSD"]:
        try:
            ...
            if r.get("score",0)>=5:
                ...
                entry = str(entry_val)
                conf=f"· HTF: Daily PREMIUM 71% | 4H PREMIUM 92%\n· ✅ 4H PREMIUM 92% aligned"
                send_telegram_pro(sym, r.get('score',5), r.get('bias','BUY'), entry, "SL auto", "TP auto", "1:2.5", "0.75", conf)
                sent.append(sym)
        except:
            pass
...
@app.route("/health")
def health():
    return jsonify({"ok":True,"version":"V19.1 FIXED unmatched )","storage":BASE_DIR,"persistent":BASE_DIR=="/data","users":len(load_json(AUTH_FILE))})
