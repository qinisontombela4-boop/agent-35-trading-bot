...
def build_signal_msg(res, user):
    ...
    sess = user.get('sessions') or user['sessions'] if isinstance(user, dict) else user.sessions
    return f"""🚀 *{res['symbol']} {res['direction']}* | {conviction} *{score}/8*
*Session:* {sess} | UTC {datetime.utcnow().strftime('%H:%M')}
*Bias:* {bias} | *RR:* {user['risk_reward'] if isinstance(user, dict) else user[0]} | *Acct:* {user['account_size'] if isinstance(user, dict) else ''}
...
_Sessions: {sess} | Red news: ✅ Checked (±60m block) | Asia=JPY/AUD | Sydney=AUD/NZD | London=Volatility | NY=USD_"""
...
.sess-check{display:flex;align-items:center;gap:8px;background:#121d30;padding:12px;border-radius:12px;border:1px solid #1e2d45;cursor:pointer}
.sess-check input{width:18px;height:18px}
</style>
<script>
...
def approve(pid):
    ...
    if not cur.fetchone() or not cur.fetchone()['is_creator'] if False else True:
        cur.execute("SELECT is_creator FROM agent35_users WHERE email=%s", (session['email'],))
        chk=cur.fetchone()
...
def delete_payment(pid):
    ...
    cur.execute("SELECT is_creator FROM agent35_users WHERE email=%s", (session['email'],)); chk=cur.fetchone()
    if not chk or not chk['is_creator']: cur.close(); conn.close(); return "Not allowed"
    cur.execute("DELETE FROM agent35_payments WHERE id=%s", (pid,)); conn.commit(); cur.close(); conn.close()
    return redirect('/master')
...
def master():
    ...
    content = f"<h2 style='color:#10b981'>MASTER V5.9 - Sessions+Reason</h2><div class='grid grid4'><div class='card'><div class='stat-label'>Users</div><div class='stat-value'>{stats['total']}</div></div><div class='card'><div class='stat-label'>Revenue</div><div class='stat-value'>R{rev_stats['rev']}</div></div><div class='card'><div style='font-size:11px'>UTC {datetime.utcnow().strftime('%H:%M')} Sessions: Sydney 22-06 Asia 0-9 London 8-16 NY 13-21<br>Red news ±60m block</div><a class='btn-outline' href='/setup-webhook'>Webhook</a></div><div class='card'><form method='GET' action='/master'><input name='q' value='{q}' placeholder='Search email/ref'><button class='btn' style='margin-top:6px'>Search</button></form></div></div><div class='card' style='margin-top:14px'>{pays_html}</div><div class='card' style='margin-top:14px;overflow:auto'><table><tr><th>User / Session</th><th>Status</th><th>Joined</th><th>Action</th></tr>{users_html}</table></div>"
    return layout(content, session['email'], "master")

@app.route('/payment')
def payment_page():
    ref = f"AG35-{datetime.now().strftime('%m%d')}-{os.urandom(2).hex().upper()}"
    content = f"<div class='card' style='max-width:500px;margin:auto;text-align:center;padding:28px'><h2 style='color:#10b981'>Upgrade</h2><p>Capitec: <b>{CAPITEC_ACC}</b></p><div style='background:#070d1a;padding:14px;border-radius:12px;border:1px solid #10b981;margin:12px 0'><div style='font-size:22px;font-weight:800;color:#10b981'>{ref}</div></div><a class='btn' href='/submit-payment?ref={ref}&plan=yearly'>I Paid R500 Yearly ({ref})</a><a class='btn-outline' href='/submit-payment?ref={ref}&plan=lifetime'>I Paid R5000 Lifetime ({ref})</a></div>"
...
def test_telegram():
    ...
    if not u or not u['telegram_id']: return layout("<div class='card'>Not Linked</div>", session['email'])
    fake_res = {"symbol":"XAUUSD","direction":"BUY","entry":2675.5,"sl":2668.0,"tp":2695.0,"score":7,"bias":"Bullish HTF","confluence":["Daily bullish","4H BOS","1H FVG tap","London liquidity sweep","RSI 55","No red news within 60m"],"reason":"HTF bullish, swept Asia low 2670 (Sydney session), FVG tap + Order Block, entry on 15m confirmation. Asia session best for JPY pairs, Sydney for AUD/NZD"}
    msg = build_signal_msg(fake_res, u)
    ok = send_telegram(u['telegram_id'], msg, trade_id=99999, stage="signal")
    return layout(f"<div class='card'><h3 style='color:#10b981'>Sent with Reason+Score!</h3><p style='font-size:12px;white-space:pre-wrap'>{msg}</p><a class='btn' href='/dashboard'>Back</a></div>" if ok else "<div class='card'>Failed</div>", session['email'])
...
def scan():
    ...
    if not is_session_active(user['sessions'] or 'London,New York'):
        cur.close(); conn.close()
        return layout(f"<div class='card' style='text-align:center'><h3 style='color:#f59e0b'>Outside your sessions</h3><p>Your sessions: {user['sessions']}<br>UTC now: {datetime.utcnow().strftime('%H:%M')}<br>Change to 24/7 or add session.</p><a class='btn' href='/settings'>Change Sessions</a></div>", session['email'])
    ...
    for sym in symbols:
        ...
        try:
            ...
            if res.get('blocked') == 'news':
                results.append({"symbol":sym,"signal":False,"blocked":res.get('reason','Red news')}); continue
        except Exception as e: res={"signal":False,"symbol":sym}
    ...
    conn.commit(); cur.close(); conn.close()
    html="".join([f"<div class='card' style='border-left:4px solid #ef4444;margin:10px 0'><b>{r['symbol']} - BLOCKED: {r.get('blocked')}</b><br><span style='font-size:11px'>Red folder news ±60min - skipping</span></div>" if r.get('blocked') else f"<div class='card' style='border-left:4px solid #10b981;margin:10px 0'><b>{r['symbol']} {r.get('direction','')} {r.get('score',0)}/8 🔥</b><br>{r.get('confluence','')}<br><i style='font-size:11px'>{r.get('reason','') or r.get('bias','')}</i><br>Entry {r.get('entry','')} SL {r.get('sl','')} TP {r.get('tp','')}</div>" if r.get('signal') else f"<div class='card' style='opacity:0.6;margin:10px 0'><b>{r['symbol']} - No Setup (score low or news)</b></div>" for r in results])
    return layout(f"<h2>Scan Results - Session {user['sessions']} | UTC {datetime.utcnow().strftime('%H:%M')}</h2>{html}<br><a class='btn' href='/journal'>Journal</a>", session['email'])
...
def cron_scan_all():
    ...
    return jsonify({"ok":True,"msg":"scan with sessions+reason+score"})
...
def settings():
    ...
    content = f"<div style='max-width:700px;margin:auto'><h2 style='color:#10b981'>Settings - Sessions + Reason</h2><form method='POST'><div class='settings-section'><div style='display:grid;grid-template-columns:1fr 1fr;gap:12px'><div><label>Currency</label><select name='currency'><option value='USD' {sel_usd}>USD</option><option value='ZAR' {sel_zar}>ZAR</option><option value='EUR' {sel_eur}>EUR</option></select></div><div><label>Account</label><input name='acc' type='number' value='{u['account_size']}'></div><div><label>Lot</label><input name='lot' type='number' step='0.01' value='{u['lot_size']}'></div><div><label>RR</label><select name='rr'><option {sel_12}>1:2</option><option {sel_13}>1:3</option><option {sel_14}>1:4</option></select></div></div><label>Symbols</label><input name='symbols' value='{u['symbols']}'><label>Telegram @</label><input name='tg' value='{u['telegram_username'] or ''}'><div style='margin-top:16px'><label>Preferred Sessions - When to send signals (UTC) - With Reason + Score</label><div style='display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px'><label class='sess-check'><input type='checkbox' name='sess_london' {c_london}> London 8-16 UTC (Best volatility)</label><label class='sess-check'><input type='checkbox' name='sess_ny' {c_ny}> New York 13-21 UTC (USD pairs)</label><label class='sess-check'><input type='checkbox' name='sess_asia' {c_asia}> Asia 0-9 UTC (JPY, AUD, Gold slow)</label><label class='sess-check'><input type='checkbox' name='sess_sydney' {c_sydney}> Sydney 22-6 UTC (AUD/NZD, early Asia)</label></div><label class='sess-check' style='margin-top:8px;background:#10b98122;border-color:#10b981'><input type='checkbox' name='sess_all' {c_all}> 24/7 - All sessions</label><div style='font-size:11px;color:#10b981;margin-top:8px;background:#070d1a;padding:10px;border-radius:8px'>🔴 RED NEWS: ±60min block auto. Bot now sends confluence 6/8, 7/8 etc + mini reason per signal.<br>Asia = JPY best | Sydney = AUD/NZD | London+NY = most signals | Conviction HIGH 7-8, MEDIUM 5-6</div></div></div><button class='btn'>Save All</button><a href='/dashboard' class='btn-outline'>Back</a></form></div>"
    ...
    try:
        ...
        if 'message' in data:
            ...
            if row:
                ...
                send_telegram(chat_id, f"✅ Linked! {row['email']}\nRef: {row['payment_ref'] or ref}\nSessions: {row.get('sessions','') if isinstance(row, dict) else ''} | Now set Asia/Sydney in Settings", trade_id=88888, stage="signal")
...
@app.route('/healthz')
def health(): return jsonify({"status":"ok","version":"V5.9","sessions":["Sydney","Asia","London","New York","24/7"],"reason_score":"yes","utc":datetime.utcnow().isoformat()})
