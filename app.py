...
def build_signal_msg(res, user=None):
    try:
        ...
        news_alert=f"\n\n{res.get('news_text','')}" if res.get('news_warning') else ""
...
def layout(content, email="", active="dashboard"):
    ...
    return f'<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>Agent35 V8.6</title>{STYLE}</head><body><div class="header"><div class="logo">{LOGO_SVG} AGENT 35 V8.6 NO-SPAM <div class="clock-bar"><span class="live-dot"></span><span id="utcClock">UTC --:--:--</span> | <span id="sastClock">SAST --:--:--</span> | <span id="sessClock" style="color:#10b981;font-weight:800">Loading</span></div></div><div><span style="font-size:11px;color:#94a3b8">{email}</span> <a href="/logout" style="color:#94a3b8;text-decoration:none;margin-left:10px">Logout</a></div></div><div style="padding:14px;max-width:1400px;margin:auto">{tabs}{content}</div><script>function updateClock(){{const now=new Date();const utc=now.toISOString().substr(11,8);document.getElementById("utcClock").innerText="UTC "+utc;try{{const sast=new Date(now.toLocaleString("en-US",{{timeZone:"Africa/Johannesburg"}}));document.getElementById("sastClock").innerText="SAST "+sast.toLocaleTimeString("en-GB");}}catch(e){{document.getElementById("sastClock").innerText="SAST "+utc;}} const h=now.getUTCHours();let s=[];if(h>=22||h<6)s.push("Sydney");if(h>=0&&h<9)s.push("Asia");if(h>=8&&h<16)s.push("London");if(h>=13&&h<21)s.push("New York");if(s.length==0)s=["Off Hours"];document.getElementById("sessClock").innerText=s.join(" + ")+" ACTIVE";}} setInterval(updateClock,1000);updateClock();</script></body></html>'
...
def guide_page():
    ...
    content="<h1 style='color:#10b981'>V8.6 NO-SPAM</h1><div class='card'><b>Journal</b> = Only TOOK trades + Money - Telegram only for TOOK<br><b>All Signals</b> = All signals for bot winrate (no TG spam)<br><b>Decimals Fix</b> = USDZAR 16.038 / 16.023 / 16.084<br><b>Spam Fix</b> = Old trades killed, only 10 active max, 24h dedup</div><div class='card' style='text-align:center'><a class='btn' href='/dashboard'>Dashboard</a></div>"
...
def test_telegram():
    ...
    msg = build_signal_msg(res if res.get('signal') else {'symbol': 'XAUUSD','direction': 'BUY','entry': 2685.5,'sl': 2675,'tp': 2705,'score': 7,'quality': 'PREMIUM Be-OB','bias': 'Bu-OB:True','confluence': ['Bu-OB RETEST','MB'],'reason': 'Test V8.6 No-Spam + Decimals Fix'}, u)
    ok = send_telegram(u['telegram_id'], f"🧪 TEST V8.6 NO-SPAM + DECIMALS 🧪\n\n{msg}", trade_id=99999, stage="signal")
    cur.close(); conn.close()
    content = f"<div class='card' style='text-align:center'><h3 style='color:#10b981'>✅ Sent V8.6!</h3><div style='background:#070d1a;padding:12px;border-radius:10px;text-align:left;font-size:12px;white-space:pre-wrap'>{msg}</div><br><a class='btn' href='/dashboard'>Back</a></div>" if ok else "<div class='card'><h3>Failed - Check Bot Token</h3></div>"
...
def scan():
    ...
    for sym in symbols:
        ...
        if res.get('signal') and res.get('score',0) >= 4:
            ...
            cur.execute("INSERT INTO agent35_trades (user_email,symbol,direction,entry,sl,tp,original_entry,original_sl,timeframe_bias,confluence,status,be_done,lock_done) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'sent',FALSE,FALSE) RETURNING id",(session['email'],res['symbol'],res['direction'],res['entry'],res['sl'],res['tp'],res['entry'],res['sl'],res['bias'],str(res.get('confluence',''))))
    ...
    return layout(f"<h2>Scan V8.6 - No Spam + Decimals Fixed</h2>{html}<br><a class='btn' href='/journal'>Journal (Taken)</a> <a class='btn-outline' href='/signals'>All Signals</a>", session['email'])
    ...
    try:
        ...
        if 'callback_query' in data:
            ...
            if tr:
                if action=='took':
                    ...
                    send_telegram(chat_id, f"📝 {tr['symbol']} TOOK - Now tracking BE, Lock, WIN/LOSS only for this trade (once)", trade_id=tid, stage="active")
                elif action=='skip':
                    cur.execute("UPDATE agent35_trades SET status='skipped', be_done=TRUE, lock_done=TRUE WHERE id=%s", (tid,)); conn.commit()
                    send_telegram(chat_id, f"Skipped {tr['symbol']} - No more TG alerts, still in All Signals sheet")
                elif action=='lock':
        ...
        if 'message' in data:
            ...
            if row:
                ...
                send_telegram(chat_id, f"Linked! {row['email']} V8.6 No-Spam Active - Only TOOK trades alert")
            cur.close(); conn.close()
...
def setup_webhook():
    ...
    return layout(f"<div class='card'><h3>Webhook V8.6</h3><p>{wh_url}</p><p>{r.text}</p><a class='btn' href='/master'>Back</a></div>", session['email'])
...
def cron_update():
    def do_update():
        try:
            conn=get_conn(); cur=conn.cursor()
            # V8.6 NO-SPAM: ONLY TOOK, 12h, 10 max, be_done FALSE = no repeat
            cur.execute("""
                ...
                AND (t.be_done IS NULL OR t.be_done = FALSE OR t.lock_done IS NULL OR t.lock_done = FALSE OR t.status='took')
                LIMIT 10
            """)
            for tr in cur.fetchall():
                ...
                if r_now >= 2.0 and not tr.get('lock_done'):
                    ...
                    if tr['telegram_id']:
                        ...
                        send_telegram(tr['telegram_id'], f"💎 {tr['symbol']} +2R! Lock +1R secured?", trade_id=tr['id'], stage="profitlock", extra_buttons=extra)
                rr=3
                try: rr=int(tr['risk_reward'].split(':')[1])
                except: pass
                ...
                if new:
                    ...
                    # V8.6 CLOSE FOREVER - be_done+lock_done TRUE stops spam
                    cur.execute("UPDATE agent35_trades SET status=%s, pnl=%s, closed_at=NOW(), result_price=%s, auto_updated=TRUE, close_r=%s, be_done=TRUE, lock_done=TRUE WHERE id=%s",(new,pnl,close,tr['id'],close_r))
                    conn.commit()
                    if tr['telegram_id']:
            ...
            print(f"cron V8.6 err {e} {traceback.format_exc()}")
    threading.Thread(target=do_update, daemon=True).start()
    return jsonify({"ok":True, "version":"V8.6 NO-SPAM"})
...
def cron_scan_all():
    def do_scan():
        try:
            conn=get_conn(); cur=conn.cursor()
            # V8.6: Only approved + respect watchlist + 24h dedup
            cur.execute("SELECT * FROM agent35_users WHERE payment_status='approved' AND symbols IS NOT NULL LIMIT 100")
            users=cur.fetchall()
            for user in users:
                ...
                for sym in symbols:
                    sym=sym.strip().upper()
                    if not sym: continue
                    # DEDUP 24h - stops pairs you don't watch spam
                    ...
                    if res.get('signal') and res.get('score',0) >= 4:
                        # INSERT AS SENT ONLY - NEVER TOOK
                        cur.execute("INSERT INTO agent35_trades (user_email,symbol,direction,entry,sl,tp,original_entry,original_sl,timeframe_bias,confluence,status,be_done,lock_done) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'sent',FALSE,FALSE) RETURNING id",(user['email'],res['symbol'],res['direction'],res['entry'],res['sl'],res['tp'],res['entry'],res['sl'],res['bias'],str(res.get('confluence',''))))
                        row=cur.fetchone()
            ...
            print(f"scan V8.6 err {e} {traceback.format_exc()}")
    threading.Thread(target=do_scan, daemon=True).start()
    return jsonify({"ok":True})

@app.route('/healthz')
def health(): return jsonify({"status":"ok","version":"V8.6-NO-SPAM-DECIMALS-FIXED","utc":datetime.utcnow().isoformat()})
...
def master():
    ...
    return layout(f"<div class='card'><h2>MASTER V8.6 - Users {stats['total']} - NO-SPAM + DECIMALS FIX</h2><a class='btn-outline' href='/setup-webhook'>Fix Webhook</a><br><br><div class='warn'>Cron: scan-all 60min, update-trades 5min ONLY</div></div>", session['email'], "master")
