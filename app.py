...
TELEGRAM_BOT_USERNAME = "Sniper035_bot" # UPDATED
...
MAP = {"EURUSD":"EURUSD=X","GBPUSD":"GBPUSD=X","USDJPY":"JPY=X","AUDUSD":"AUDUSD=X","USDCAD":"CAD=X","USDCHF":"CHF=X","NZDUSD":"NZDUSD=X","EURGBP":"EURGBP=X","EURJPY":"EURJPY=X","GBPJPY":"GBPJPY=X","USDZAR":"ZAR=X","EURZAR":"EURZAR=X","GBPZAR":"GBPZAR=X","XAUUSD":"GC=F","GOLD":"GC=F","XAGUSD":"SI=F","USOIL":"CL=F","UKOIL":"BZ=F","BTCUSD":"BTC-USD","ETHUSD":"ETH-USD","SOLUSD":"SOL-USD","NAS100":"^NDX","US30":"^DJI","SPX500":"^GSPC","GER40":"^GDAXI","UK100":"^FTSE","JP225":"^N225","AAPL":"AAPL","TSLA":"TSLA","NVDA":"NVDA","MSFT":"MSFT"}
...
def auto_update_trades():
    while True:
        try:
            conn = get_conn(); cur = conn.cursor()
            cur.execute("SELECT t.*, u.account_size, u.risk_reward FROM agent35_trades t JOIN agent35_users u ON u.email=t.user_email WHERE t.status IN ('sent','took','active') LIMIT 50")
            for tr in cur.fetchall():
                ...
                if tr['status']=='sent' and low <= tr['entry'] <= high: new_status='took'
...
.header{background:rgba(11,17,28,0.95);backdrop-filter:blur(10px);border-bottom:1px solid #1a2535;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:100}
...
<script>
const ALL_SYMBOLS = ["EURUSD","GBPUSD","USDJPY","AUDUSD","USDZAR","XAUUSD","GOLD","BTCUSD","ETHUSD","NAS100","US30","GER40","USOIL","AAPL","TSLA"];
...
def dashboard():
    ...
    <div class='grid grid4'>
      <div class='card'><div class='stat-label'>Total PNL Auto</div><div class='stat-value'>{curr_sym}{round(pnl,2)} <span class='badge {"bull" if pnl>=0 else "bear"}'>{round(wr,1)}% WR</span></div><div style='font-size:11px;color:#10b981;margin-top:6px'>● Auto every 3 min</div></div>
      ...
      <div class='card' style='position:relative'><div class='stat-label'>Watchlist (5 max)</div><div style='margin:10px 0'>{chips or 'No symbols'}</div><form id='symForm' method='POST' action='/quick-symbols'><input type='hidden' name='symbols' id='symInput' value="{user['symbols']}"></form><input id='symSearch' class='searchbox' placeholder='🔍 Search e.g. ZAR...' oninput='filterSyms()'><div id='symDropdown' class='dropdown'></div><div style='margin-top:8px'>{pop_btns}</div></div>
      <div class='card'><div class='stat-label'>Actions • @Sniper035_bot</div><a class='btn' href='/scan' style='margin-top:10px'>🔍 SCAN MARKET</a><a class='btn-outline' href='/journal'>📔 Open Journal</a><a class='btn-outline' href='https://t.me/{TELEGRAM_BOT_USERNAME}?start={session['email']}' target='_blank' style='border-color:#10b981;color:#10b981'>📲 { '✅ @'+user['telegram_username'] if user['telegram_username'] else '❌ Not linked - Tap' }</a></div>
    </div>
    <div class='card' style='margin-top:14px'><div style='display:flex;justify-content:space-between'><h3 style='margin:0;color:#10b981'>Recent Trades (Auto)</h3><a href='/journal' style='font-size:12px'>Full Journal →</a></div><div style='overflow:auto;margin-top:12px'><table><tr><th>Time</th><th>Symbol</th><th>Dir</th><th>Entry</th><th>Status</th><th>PNL</th></tr>{''.join([f"<tr><td>{t['created_at'].strftime('%m-%d %H:%M')}</td><td><b>{t['symbol']}</b></td><td><span class='badge { 'bull' if t['direction']=='BUY' else 'bear'}'>{t['direction']}</span></td><td>{round(t['entry'],4)}</td><td><span class='badge { 'win' if t['status']=='win' else 'loss' if t['status']=='loss' else ''}'>{t['status'].upper()}</span></td><td style='font-weight:800;color:{'#10b981' if t['pnl']>0 else '#ef4444' if t['pnl']<0 else '#94a3b8'}'>{curr_sym}{round(t['pnl'],2)}</td></tr>" for t in trades]) or '<tr><td colspan=6 style=text-align:center;color:#475569> No trades </td></tr>'}</table></div></div>
    """
...
def journal():
    ...
    <div class='grid grid4'>
      ...
      <div class='card'><div class='stat-label'>Win Rate</div><div class='stat-value'>{round(wr,1)}%</div><div style='font-size:12px;color:#64748b'>{s['closed']} closed</div></div>
      <div class='card'><div class='stat-label'>Total Trades</div><div class='stat-value'>{s['total_trades']}</div><div style='font-size:12px;color:#10b981'>● Live auto-update</div></div>
      <div class='card'><div class='stat-label'>Export</div><a href='/export/csv' class='btn' style='margin-top:10px'>📥 Export to Excel (CSV)</a><div style='font-size:11px;color:#475569;margin-top:6px'>Download all trades for Excel / Sheets</div></div>
    </div>
    <div class='card' style='margin-top:14px'><div style='display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px'><h3 style='margin:0;color:#10b981'>📔 Full Trading Journal - Auto Updates When Trade Closes</h3><div style='display:flex;gap:8px'><a href='/export/csv' class='btn' style='width:auto;padding:8px 14px;font-size:12px'>📥 CSV</a><a href='/cron/update-trades' class='btn-outline' style='width:auto;padding:8px 14px;font-size:12px'>🔄 Force Update</a></div></div>
    <div style='overflow:auto;margin-top:12px'><table><tr><th>Date</th><th>Symbol</th><th>Dir</th><th>Entry / SL / TP</th><th>Result</th><th>Status</th><th>PNL</th><th>Held</th></tr>
    {''.join([f"<tr><td>{t['created_at'].strftime('%Y-%m-%d')}<br><span style='font-size:11px;color:#475569'>{t['created_at'].strftime('%H:%M')}</span></td><td><b>{t['symbol']}</b></td><td><span class='badge { 'bull' if t['direction']=='BUY' else 'bear'}'>{t['direction']}</span></td><td style='font-size:11px'>E {round(t['entry'],4)}<br>SL {round(t['sl'],4)}<br>TP {round(t['tp'],4)}</td><td>{round(t['result_price'],4) if t['result_price'] else '-'}<br><span style='font-size:10px;color:#475569'>{t['closed_at'].strftime('%H:%M') if t['closed_at'] else ''}</span></td><td><span class='badge { 'win' if t['status']=='win' else 'loss' if t['status']=='loss' else 'bull'}'>{t['status'].upper()}</span>{'<div style=font-size:10px;color:#10b981>🤖 Auto</div>' if t['auto_updated'] else ''}</td><td style='font-weight:800;color:{'#10b981' if t['pnl']>0 else '#ef4444' if t['pnl']<0 else '#94a3b8'}'>{curr_sym}{round(t['pnl'],2)}</td><td style='font-size:11px;color:#64748b'>{(str(t['closed_at']-t['created_at']).split('.')[0] if t['closed_at'] else 'Active')[:7]}</td></tr>" for t in trades]) or '<tr><td colspan=8 style=text-align:center;padding:30px;color:#475569>No trades yet</td></tr>'}
    </table></div></div>
    """
...
def export_csv():
    ...
    writer.writerow(['Date','Time','Symbol','Direction','Entry','StopLoss','TakeProfit','Result Price','Status','PNL '+user['currency'],'Confluence','Bias','Closed At','Time Held','Auto Updated'])
    for t in trades:
        held = str(t['closed_at']-t['created_at']).split('.')[0] if t['closed_at'] else 'Active/Waiting'
        writer.writerow([t['created_at'].strftime('%Y-%m-%d'), t['created_at'].strftime('%H:%M:%S'), t['symbol'], t['direction'], t['entry'], t['sl'], t['tp'], t['result_price'] or '', t['status'], t['pnl'], t['confluence'] or '', t['timeframe_bias'] or '', t['closed_at'].strftime('%Y-%m-%d %H:%M:%S') if t['closed_at'] else '', held, 'Yes' if t['auto_updated'] else 'No'])
    csv_data = output.getvalue()
    return Response(csv_data, mimetype='text/csv', headers={'Content-Disposition': f'attachment; filename=agent35_journal_{session["email"]}_{datetime.now().strftime("%Y-%m-%d")}.csv'})
...
def settings():
    ...
    return layout(f"""
    <div style='max-width:800px;margin:auto'><h2 style='color:#10b981'>⚙️ Professional Settings</h2>
      <form method='POST'>
        <div class='settings-section'><h4>💱 ACCOUNT CONFIGURATION</h4>
          <div class='grid' style='grid-template-columns:1fr 1fr;gap:12px'>
            <div><label>Currency</label><select name='currency'>{opts}</select></div>
            <div><label>Account Size</label><input name='acc' type='number' value="{u['account_size']}"></div>
            <div><label>Lot Size</label><input name='lot' type='number' step='0.01' value="{u['lot_size']}"></div>
            <div><label>Leverage</label><select name='lev'><option {'selected' if u['leverage']=='1:100' else ''}>1:100</option><option {'selected' if u['leverage']=='1:500' else ''}>1:500</option><option {'selected' if u['leverage']=='1:1000' else ''}>1:1000</option></select></div>
          </div>
        </div>
        <div class='settings-section'><h4>📊 RISK MANAGEMENT</h4>
          <div class='grid' style='grid-template-columns:1fr 1fr;gap:12px'>
            <div><label>Risk : Reward</label><select name='rr'><option {'selected' if u['risk_reward']=='1:2' else ''}>1:2</option><option {'selected' if u['risk_reward']=='1:3' else ''}>1:3</option><option {'selected' if u['risk_reward']=='1:4' else ''}>1:4</option></select></div>
            <div><label>Sessions</label><input name='sessions' value="{u['sessions']}"></div>
          </div>
          <label>Watchlist Symbols</label><input name='symbols' value="{u['symbols']}">
        </div>
        <div class='settings-section'><h4>🔗 TELEGRAM • @Sniper035_bot</h4>
          <label>Telegram Username</label><input name='tg' value="{u['telegram_username'] or ''}" placeholder="@username">
          <div style='margin-top:10px;padding:10px;background:#070d1a;border-radius:10px'><span style='font-size:12px'>Bot Link: </span><a href='https://t.me/{TELEGRAM_BOT_USERNAME}?start={session['email']}' target='_blank' style='color:#10b981'>https://t.me/{TELEGRAM_BOT_USERNAME}</a></div>
        </div>
        <button class='btn'>💾 Save Settings</button><a href='/dashboard' class='btn-outline'>Back</a>
      </form>
    </div>
...
def scan():
    ...
    conn.commit(); cur.close(); conn.close()
    html="".join([f"<div class='card' style='border-left:4px solid #10b981;margin:10px 0'><b>{r['symbol']} {r.get('direction','')} {r.get('score',0)}/8</b><br>{r.get('entry','')} SL {r.get('sl','')} TP {r.get('tp','')}</div>" if r.get('signal') else f"<div class='card' style='opacity:0.6;margin:10px 0'><b>{r['symbol']} - No Setup</b></div>" for r in results])
    return layout(f"<h2>Scan Results</h2>{html}<br><a class='btn' href='/journal'>View Journal (Auto Updates)</a><a class='btn-outline' href='/dashboard'>Dashboard</a>", session['email'])

@app.route('/cron/update-trades')
def cron_update():
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("SELECT t.*, u.account_size, u.risk_reward FROM agent35_trades t JOIN agent35_users u ON u.email=t.user_email WHERE t.status IN ('sent','took') LIMIT 30")
        for tr in cur.fetchall():
            live=get_live_price(tr['symbol'])
            if not live: continue
...
@app.route('/healthz')
def health(): return jsonify({"status":"ok","bot":"@Sniper035_bot V5.1 CSV"})
